...
def llamar_TP():
    ...
    for i, demanda in enumerate(valores_tipo_auto):
        probabilidad = float(entry_probabilidad_tipo[i].get())  # Convertir a float
    ...
    for i, demanda in enumerate(valores_tipo_minutos):
        ...
        #    messagebox.showerror("Error", f"El precio en la fila {i + 1} debe ser mayor a 0")
        #    return
        probabilidades_minutos.append(probabilidad)
        #precios_unitarios.append(precio)
        demandas_minutos.append(demanda)

    # Validar que la suma de las probabilidades sea igual a 1
    if round(sum(probabilidades_tipo), 2) != 1.00:
        messagebox.showerror("Error", "La suma de las probabilidades debe ser igual a 1")
        return

    # Calcular las probabilidades acumuladas
    prob_acumuladas = calcular_probabilidades_acumuladas(probabilidades_tipo)
    ...
    tabla_completa, promedio_pastelitos_sobrantes, promedio_utilidad = generar_simulacion_completa(prob_acumuladas, demandas_tipo, precios_unitarios, cantidad_dias)
